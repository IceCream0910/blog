 ...
 */
export declare type Node<Type = string> = {
    title?: string;
    id: string;
    /**
     * parent node's id
...
export declare type StringCoords = {
    x: number;
    y: number;
};
export declare type WithCoords<T> = T & Coords;
export declare type WithStringCoords<T> = T & StringCoords;
export declare type WithPartialCoords<T> = T & Partial<Coords>;
export declare type LinkWithPartialCoords = {
    source: WithPartialCoords<Link[`source`]>;
    target: WithPartialCoords<Link[`target`]>;
};
export declare type LinkWithCoords<L extends Link = Link> = {
...
export declare type SmallestNextVisibilityInput = {
    data: string[];
    name: string;
    renderAll?: boolean;
    array: string[];
};
export declare type NotSmallestNextVisibilityInput = {
    data: Set<string>;
    name: string;
    renderAll?: boolean;
    array: string[];
};
export declare type NextVisibilityInput = SmallestNextVisibilityInput | NotSmallestNextVisibilityInput;
export declare type KnowledgeGraphOptions<N extends WithPartialCoords<Node> = WithPartialCoords<Node>, L extends LinkWithPartialCoords = LinkWithPartialCoords> = {
    optimization?: {
        /**
...
export declare type NodeLabel<N extends Node = Node> = BitmapNodeLabel<Node> | VectorNodeLabel<Node>;
export declare type CustomFontConfig = NonNullable<KnowledgeGraphOptions[`graph`]>[`customFont`];
