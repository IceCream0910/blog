...
interface GraphEventCallbacks<N extends Node = Node, L extends Link = Link> {
    ...
    finishGraph(nodesAndLinks: {
        nodes: WithCoords<N>[];
        links: L[];
    }): void;
    finishLabels(nodes: WithCoords<N>[]): void;
    finishLayout(nodesAndLinks: {
        nodes: WithCoords<N>[];
        links: L[];
    }): void;
    error(e: Error): void;
}
export declare type GraphEvents = EventKey<GraphEventCallbacks>;
export declare class GraphEventEmitter<N extends WithCoords<Node> = WithCoords<Node>, L extends LinkWithCoords = LinkWithCoords> extends EE<GraphEventCallbacks<N, L>> {
}
export {};
